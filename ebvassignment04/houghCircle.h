  ...
  */
  void hough (Mat_<ushort>& houghImg, const Mat_<ushort>& sobelImgPrev, const Mat_<ushort>& sobelImg) const;
   ...
   /*! Finds circle in the hough image by running through the image and adding a circle for every
       ...
       For each circle added the best radius is determined using \c findBestRadius and
       only those circles ares finally added which have a value in the 
       hough-radius-accumulator above \c radiusHoughThreshold. For testing the
