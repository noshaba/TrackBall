...
public:
    ...
    Interval() :
            xLo(0), xHi(0), y(0), region(0), parent(NULL)
    {
    }
    ;
    Interval(int xLo, int xHi, int y) :
            xLo(xLo), xHi(xHi), y(y), region(0), parent(NULL)
    {
    }
};
...
public:
     ...
     */
    void
    thresholdAndRLE(Mat_<uchar>& image, uchar threshold, int minLength);
...
void
RegionSet::thresholdAndRLE(Mat_<uchar>& image, uchar threshold, int minLength)
{
    unsigned int length;
...
void
RegionSet::pathCompress(Interval* iv)
{
...
void
RegionSet::unite(Interval* iv1, Interval* iv2)
{
    pathCompress(iv1);
    pathCompress(iv2);
    iv1->parent < iv2->parent ? iv2->parent->parent = iv1->parent : iv1->parent->parent =
                                        iv2->parent;
}

void
RegionSet::initialize()
{
...
bool
RegionSet::touch(Interval* run, Interval* flw)
{
    return run->y == flw->y + 1 && run->xHi >= flw->xLo && flw->xHi >= run->xLo;
}

bool
RegionSet::ahead(Interval* run, Interval* flw)
{
...
public:
    ...
    Region() :
            integral(0), integralX(0), integralY(0), integralXX(0),
            integralXY(0), integralYY(0), centerX(0), centerY(0), mainAxis(0),
            smallLength(0), largeLength(0)
    {
    }
     ...
     */
    static void
    computeMoments(vector<Region>& region, const RegionSet& decomposition);
...
protected:
     ...
     */
    static void
    eigenDecompositionSymmetric(const Matrix2x2& a,
                                double& phi,
                                double& l0,
                                double& l1);
};
...
void
Region::eigenDecompositionSymmetric(const Matrix2x2& a,
                                    double& phi,
                                    double& l0,
                                    double& l1)
{
...
void
Region::computeMoments(vector<Region> &region, const RegionSet &decomposition)
{
    ...
    for (unsigned int i = 0; i < rle.size(); ++i)
    {
        ...
        region[I.region].integralX +=
                (I.xHi * (I.xHi + 1) - I.xLo * (I.xLo - 1)) * .5;
        region[I.region].integralY += (I.xHi - I.xLo + 1) * I.y;
        region[I.region].integralXX += (std::pow(I.xHi + .5, 3)
                - std::pow(I.xLo - .5, 3)) / 3.0;
        region[I.region].integralXY += (I.xHi * (I.xHi + 1)
                - I.xLo * (I.xLo - 1)) * I.y * .5;
        region[I.region].integralYY += (I.xHi - I.xLo + 1)
                * (I.y * I.y + 1.0 / 12.0);
    }
}

void
Region::computeFeatures()
{
    ...
    eigenDecompositionSymmetric( { { dIntegralXX, dIntegralXY }, { dIntegralXY,
        dIntegralYY } }, mainAxis, eig1, eig2);
 ...
 */
void
paintDecomposition(Mat& img, const RegionSet& decomposition)
{
    Scalar color[6] = { CV_RGB(255, 0, 0), CV_RGB(0, 255, 0), CV_RGB(255, 255,
            0), CV_RGB(0, 0, 255), CV_RGB(255, 0, 255), CV_RGB(0, 255, 255) };
 ...
 */
void
paintFeatures(Mat& img, const vector<Region>& features)
{
    for (int i = 0; i < (int) features.size(); i++)
    {
        ...
        line(img,
                Point((int) (rg->centerX + rg->largeLength * c),
                        (int) (rg->centerY + rg->largeLength * s)),
                Point((int) (rg->centerX - rg->largeLength * c),
                        (int) (rg->centerY - rg->largeLength * s)),
                CV_RGB(255, 255, 255));
        // paint +/-smallLength in direction (-s,c) orthogonal to (c,s) from the center of gravity
        line(img,
                Point((int) (rg->centerX - rg->smallLength * s),
                        (int) (rg->centerY + rg->smallLength * c)),
                Point((int) (rg->centerX + rg->smallLength * s),
                        (int) (rg->centerY - rg->smallLength * c)),
                CV_RGB(255, 255, 255));
