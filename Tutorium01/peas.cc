...
void findPeas (IplImage* img, vector<Pea>& pea)
{
	...
	for(int i = 0; i < img->width; i++) {
		for(int j = 0; j < img->height; j++) {
			if(pixel(img, i, j) <= 100) {
				if(j+10 <= img->height-1 && pixel(img, i, j+10) <= 100) {
					alreadyListed = false;
					for(std::vector<Pea>::iterator it = pea.begin(); it != pea.end(); ++it) {
						if(((*it).x <= i+25 && (*it).x >= i-25) &&
							((*it).y <= j+25 && (*it).y >= j-25)) {
							alreadyListed = true;
						}
					}
					if(!alreadyListed) {
						if(i+10 < img->width && i > 10) {
							pea.push_back(Pea(i+10, j+5));
						} else {
							pea.push_back(Pea(i, j+5));
						}
					}
				}
			}
...
{    
  cvNamedWindow ("Counting the number of peas", CV_WINDOW_NORMAL);
